...
#include "config/omotenashicoin-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("omotenashicoin-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "omotenashicoin.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "omotenashicoind.pid"));
#endif
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 12181, 12281));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, omotenashicoin, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all OMOTENASHICOIN specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:12181"));
...
//    strUsage += "  -anonymizeomotenashicoinamount=<n>     " + strprintf(_("Keep N MTNS anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 12180, 12280));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2018-%i OmotenashiCoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("omotenashicoin-loadblk");
...
/** Sanity checks
 *  Ensure that OMOTENASHICOIN is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. OMOTENASHICOIN Core is shutting down."));
...
#endif
    // Make sure only a single OMOTENASHICOIN process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. OMOTENASHICOIN Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("OMOTENASHICOIN version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //OMOTENASHICOIN specific: zerocoin and spork DB's
                ...
                // OMOTENASHICOIN: load previous sessions sporks if we have them.
                ...
                // OMOTENASHICOIN: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of OMOTENASHICOIN Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart OMOTENASHICOIN Core to complete") << "\n";
...
// XX42 Remove/refactor code below. Until then provide safe defaults
    nAnonymizeOmotenashiCoinAmount = 2;
...
//    }
//
//    nAnonymizeOmotenashiCoinAmount = GetArg("-anonymizeomotenashicoinamount", 0);
//    if (nAnonymizeOmotenashiCoinAmount > 999999) nAnonymizeOmotenashiCoinAmount = 999999;
//    if (nAnonymizeOmotenashiCoinAmount < 2) nAnonymizeOmotenashiCoinAmount = 2;
    ...
    LogPrintf("Anonymize OMOTENASHICOIN Amount %d\n", nAnonymizeOmotenashiCoinAmount);
