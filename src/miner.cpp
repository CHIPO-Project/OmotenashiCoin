...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("OmotenashiCoinMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("OmotenashiCoinMiner : ProcessNewBlock, block not accepted");
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    CValidationState state;
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("OMOTENASHICOINMiner : ProcessBlock, block not accepted");
...
{
    LogPrintf("OMOTENASHICOINMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("omotenashicoin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running OMOTENASHICOINMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
