...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeOmotenashiCoinAmount"))
        settings.setValue("nAnonymizeOmotenashiCoinAmount", 1000);

    nAnonymizeOmotenashiCoinAmount = settings.value("nAnonymizeOmotenashiCoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeOmotenashiCoinAmount"))
        SoftSetArg("-anonymizeomotenashicoinamount", settings.value("nAnonymizeOmotenashiCoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in omotenashicoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeOmotenashiCoinAmount:
            return QVariant(nAnonymizeOmotenashiCoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeOmotenashiCoinAmount:
            nAnonymizeOmotenashiCoinAmount = value.toInt();
            settings.setValue("nAnonymizeOmotenashiCoinAmount", nAnonymizeOmotenashiCoinAmount);
            emit anonymizeOmotenashiCoinAmountChanged(nAnonymizeOmotenashiCoinAmount);
            break;
        case CoinControlFeatures:
