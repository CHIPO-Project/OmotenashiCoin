...
#include "ui_zomocontroldialog.h"
...
std::list<std::string> ZOmoControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZOmoControlDialog::listMints;

ZOmoControlDialog::ZOmoControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZOmoControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZOmoControlDialog::~ZOmoControlDialog()
{
    delete ui;
}

void ZOmoControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZOmoControlDialog::updateList()
{
...
void ZOmoControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZOmoControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZOmo_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZOmoControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZOmoControlDialog::GetSelectedMints()
{
...
void ZOmoControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
