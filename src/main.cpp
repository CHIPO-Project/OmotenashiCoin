...
#if defined(NDEBUG)
#error "OMOTENASHICOIN cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "MtnsNet Signed Message:\n";
...
int GetBlockOffset(int nHeight)
{
    ...
    int tBlockStep = 45000; // about 1.5 month.
    int offsetKind = 9;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from OMOTENASHICOIN
...
{
    RenameThread("omotenashicoin-scriptch");
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // OMOTENASHICOIN: We use certain sporks during IBD, so check to see if they are
